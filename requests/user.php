...
switch ($_POST['action']) {
	...
	case 'add_interest':
		...
		$check = checkInterestsExist($name);	
	...
	case 'update':	
		...
		$date = date('m/d/Y', time());		$from = new DateTime($year.'-'.$month.'-'.$day);		$to   = new DateTime('today');		$age = $from->diff($to)->y;	
	...
	case 'register':	
		...
		if($email_check->num_rows == 1 ){
			...
			if ($mysqli->query($query) === TRUE) {
				...
				getUserInfo($last_id);
				/*
				$url = 'https://www.lemoncat/tinelo/requests/api.php?action=getUsers&g=2&a=18,31&c=united%20states&pc=123';
				$ch = curl_init($url);
