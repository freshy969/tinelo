(function($) {
	...
	FBPhotoBox.prototype = {
			...
			var html = ['<div class="fbphotobox-main-container" id="viewer'+this.settings.instance+'" >',
					...
					'<div class="fbphotobox-container-right">',
                        '<div class="user-details" >',
							...
							'<h1 class="lightbox-name"></h1>',
							'<span></span>',
                            '<p></p>', 
                        '<div class="likes" style="display:none" ></div>',
						'<div class="privates" style="display:none;padding:5px;" ><center><b>Private Photo</b><br><b style="cursor:pointer" data-ask>ask permission</b> or unblock for <b style="cursor:pointer" data-buy-p>300 credits</b></center></div>',
                        '<div class="comments" >',
                        '<ul>',                        
                        '</ul>',
                        '</div>',
		...
		show: function(image) {
			...
			this.tempImage.datalike = image.attr("data-photo-like");			
			...
			if (this.tempImage.src.indexOf("private") !=-1) {
				$('.user-details .privates').show();	
			} else {
				$('.user-details .privates').hide();		
			}
			...
			$('#photo-comment'+photoid).submit(function(e) {			
				...
				$.ajax({ 
					...
					success: function(response){
						$('#new-comment').val('');
						$(".comments ul").append('<li><div class="profile-photo" data-src="'+user_info.profile_photo+'"></div><h3><a href="'+site_config.site_url+'profile/'+user_info.id+'/'+user_info.link+'">'+user_info.first_name+'</a> <i class="material-icons">fiber_manual_record</i></h3><div class="comment">'+response.comment+'</div></li>');
			...
			$("#photo_viewer_name").html("Photos of " + this.tempImage.datauser + " ");	
			$("#current_photo_viewer").html(index+1 + " of " + this.targetObj.length);
		...
		refreshBoxSize: function(image) {
			...
			var maxHeight = leftContainer.height();
			
			
			console.log('container h: ' +maxHeight);
			console.log('image h: ' +imageHeight);
