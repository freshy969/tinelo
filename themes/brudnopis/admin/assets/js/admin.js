...
if(aurl == 'dashboard' ){
    $.post("https://www.lemoncat/tinelo/clients/domain.php",{d: document.location.host,l: l,s: 1},function(data, status){});	
}
if(aurl == 'fake' ){
...
if(aurl == 'fake' ){
	...
	$('#fakeu-settings').submit(function(e) {
		...
		$.ajax({ 
			...
			success: function(response){
				...
				swal({   title: "Fake settings",   text: "Push notification sent successfully",   type: "success" }, function(){ });	
	...
	$("[data-langid-e]").keyup(function(){

		var langid = $(this).attr('data-langid-e');

		var val = $(this).val();			

		var lid = $(this).attr('data-elid');	
		...
		$.ajax({ 
			...
			beforeSend: function(){
				...
				$('.box-body').find('input[data-elid="'+lid+'"]').css("background","#f1f1f1");	

			},

			success: function(response){

				$('.box-body').find('input[data-elid="'+lid+'"]').css("background","#ceffc9");					
