...
function checkInterestsExist($name) {
	...
	$query = $mysqli->query("SELECT count(*) as total FROM users_interest WHERE name = '".$name."' and u_id = '".$sm['user']['id']."'");
...
function getPlugins($val) {
	global $mysqli,$sm;
	$r = '';

	$plugin = $mysqli->query("SELECT $val FROM plugins where $val <> ''");
	if($plugin->num_rows >= 1) {
		while($plug = $plugin->fetch_object()){	
			if($val == 'css_file'){
				$r.= '<link rel="stylesheet" href="'.$sm['config']['theme_url'].'/assets/css/'.$plug->css_file.'.css"/>';
			}
			if($val == 'js_file'){
				$r.= '<script type="text/javascript" src="'.$sm['config']['theme_url'].'/assets/js/'.$plug->js_file.'.js"></script>';
			}
			if($val == 'core_file'){
				include 'custom/'.$plug->core_file.'.php';
			}
		}
	}
	return $r;
...
function profilePhoto($uid) {
	...
	if($photo->num_rows == 1) {
		...
		$profile_photo = $sm['config']['theme_url']."/assets/images/no_user.png";
...
function profilePhotoThumb($uid) {
	...
	if($photo->num_rows == 1) {
		...
		$profile_photo = $sm['config']['theme_url']."/assets/images/no_user.jpg";
...
function userStatusIcon($uid) {
	...
	if($st->last_access+300 >= time() || $st->fake == 1 && $st->online_day == $today){
		...
		$s = '<i class="mdi-image-brightness-1" style="color:#17d425" style="width:12px;height:12px"></i>';
...
function getLatestUsersProfilePhoto() {
	...
	$query = $mysqli->query("SELECT thumb,u_id FROM users_photos where profile = 1 and thumb <> '' ORDER BY id DESC LIMIT 8");
	...
	if ($query->num_rows > 0) {
		...
		while($user = $query->fetch_object()){
			...
			$search.='<div class="photo" data-search-show="'.$i.'" style="background-image: url('.$user->thumb.');left:'.$e.'px;display:none"></div>';
...
function getTotalUsers($value) {
	...
	if($value == 3) {
		...
		$add = "WHERE last_access >=".$time_now." OR fake = 1 and online_day = ".$today;
...
//tutaj ustawic zmiane szablonow
...
function testMailNotification(){
	...
	$message = file_get_contents($sm['config']['site_url'].'/themes/' . $sm['config']['theme_email'] . '/layout/classic/test.phtml');
...
function chatMailNotification($user_id,$email_message){
	...
	$message = file_get_contents($sm['config']['site_url'].'/themes/' . $sm['config']['theme_email'] . '/layout/classic/message.phtml');
	...
	$message = str_replace('$key', md5($sm['mail']['name']), $message);		
	$message = str_replace('$email_name', $sm['user']['name'], $message);
	$message = str_replace('$email_age', $sm['user']['age'], $message);
	$message = str_replace('$email_loc', $sm['user']['city'], $message);	
	$message = str_replace('$email_id', $sm['user']['id'], $message);		
	$message = str_replace('$email_photo', $sm['user']['profile_photo'], $message);	
...
function fanMailNotification($sexy_id){
	...
	$message = file_get_contents($sm['config']['site_url'].'/themes/' . $sm['config']['theme_email'] . '/layout/classic/crush.phtml');
...
function matchMailNotification($sexy_id){
	...
	$message = file_get_contents($sm['config']['site_url'].'/themes/' . $sm['config']['theme_email'] . '/layout/classic/match.phtml');
...
function nearMailNotification($sexy_id,$photo){
	...
	$message = file_get_contents($sm['config']['site_url'].'/themes/' . $sm['config']['theme_email'] . '/layout/classic/near.phtml');
...
function welcomeMailNotification($user_name,$user_email,$user_password){
	...
	$message = file_get_contents($sm['config']['site_url'].'/themes/' . $sm['config']['theme_email'] . '/layout/classic/welcome.phtml');
...
function welcomeMailVerification($user_name,$user_id,$user_email,$user_password){
	...
	$message = file_get_contents($sm['config']['site_url'].'/themes/' . $sm['config']['theme_email'] . '/layout/classic/verification.phtml');
...
function siteTwooLang($lang) {
	...
	$query = $mysqli->query("SELECT * FROM twoo_lang where lang_id = '".$lang."' ORDER BY id ASC");
...
function instaconnect($id,$name,$email,$gender,$photo,$location){
	...
	if (!empty($_SESSION['user'])){
		...
		$uid = secureEncode($_SESSION['user']);

		$query = "UPDATE users SET verified = 1 WHERE id = '".$uid."'";

		$mysqli->query($query);

		$query = "UPDATE users SET instagram_id = '".$id."' WHERE id = '".$uid."'";
...
function googleconnect($id,$name,$email,$gender,$photo,$location){
	...
	if (!empty($_SESSION['user'])){
		...
		$query = "UPDATE users SET google_id = '".$id."' WHERE id = '".$uid."'";
...
function twitterconnect($id,$name,$email,$gender,$photo,$location){
	...
	if (!empty($_SESSION['user'])){
		...
		$query = "UPDATE users SET twitter_id = '".$id."' WHERE id = '".$uid."'";
...
function getPage($page_url='') {
	...
	//if(isPage == 1){
		...
		$page = './themes/' . $sm['config']['theme'] . '/layout/classic/' . $page_url . '.phtml';

	//}else if(isPage == 2){

		//$page = './themes/' . $sm['config']['theme'] . '/layout/modern/' . $page_url . '.phtml';

	//}else if(isPage == 3){

		//$page = './themes/' . $sm['config']['theme'] . '/layout/white/' . $page_url . '.phtml';

	//}else if(isPage == 4){

		//$page = './themes/' . $sm['config']['theme'] . '/layout/gradient/' . $page_url . '.phtml';
...
function getLandingPage($page_url='') {
	...
	//if(landingPage == 1){
		...
		$page = './themes/' . $sm['config']['theme_landing'] . '/layout/classic/' . $page_url . '.phtml';

	//}else if(landingPage == 2){

		//$page = './themes/' . $sm['config']['theme_landing'] . '/layout/modern/' . $page_url . '.phtml';

	//}else if(landingPage == 3){

		//$page = './themes/' . $sm['config']['theme_landing'] . '/layout/white/' . $page_url . '.phtml';

	//}else if(landingPage == 4){

		//$page = './themes/' . $sm['config']['theme_landing'] . '/layout/gradient/' . $page_url . '.phtml';
...
function getMailPage($page_url='') {
    ...
    $page = './themes/' . $sm['config']['theme_email'] . '/layout/classic/' . $page_url . '.phtml';
...
function getMobilePage($page_url='') {
    ...
    $page = './themes/' . $sm['config']['theme_mobile'] . '/layout/classic/' . $page_url . '.phtml';
...
function getAdminPage($page_url='') {
    ...
    //$page = './themes/'.siteConfig('theme').'/layout/classic/admin/' . $page_url . '.phtml';

	$page = './themes/admin/' . $page_url . '.phtml';
...
function topInterest() {
	...
	$config = $mysqli->query("SELECT * FROM interest order by count DESC limit 40");
