	...
	if ($spotlight->num_rows > 0) { 
		...
		while($spotl = $spotlight->fetch_object()){
			...
			if($spotl->photo == 1 ){
				...
				if (strpos($spotl->message, 'giphy') !== false) {
...
function getUserSuperLikes($uid){	
	...
	$query = $mysqli->query("SELECT sexy FROM users WHERE id = '".$uid."'"); 
	...
	if ($gift->num_rows > 0) { 
		...
		while($gi = $gift->fetch_object()){
			...
			$gifts[] = array(
				...
				"icon" => $sm['config']['site_url'].'/'.$gi->icon
...
function userAppPhotos($id) {

	global $mysqli,$sm;

	$result=array();

	$config = $mysqli->query("SELECT * FROM users_photos where u_id = '".$id."' and approved = 1 order by profile desc");
